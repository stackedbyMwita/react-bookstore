...
const News = () => {
  return (
    <div>
      <h2 className='text-3xl font-semibold py-3'>News</h2>
...

        {
          news.map((item, index) => (
            <SwiperSlide key={index}>
              <div className='flex flex-col sm:flex-row sm:justify-between items-center gap-12 border p-6 bg-slate-300 rounded-md shadow-md'>
                <div className='py-4'>
                  <Link to='/'>
                    <h3 className='text-lg font-bold text-blue-950 hover:text-blue-500'>{item.title}</h3>
                  </Link>
                  <div className='w-20 h-1 mt-2 bg-gradient-to-r from-yellow-400 to-blue-950'></div>
